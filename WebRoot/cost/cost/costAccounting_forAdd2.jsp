	...
	<head>
		...
		$(function() {
			...
			$("#sellProfit").focus(function(){
				//销售利润  = 合同价格  - 铁汽运费 - 销售成本
				if(null != finalPrice && "" != finalPrice && null != tqyf && "" != tqyf && null != sellCost && "" != sellCost) {
					var temp = parseInt(finalPrice.value) - parseInt(tqyf.value) - parseInt(sellCost.value);
			...
			$("#submitBtn").click(function() {
				...
				if(!validateNull(finalPrice,'合同价格不能为空')) {
					return false;
				}else if(!validateNumber(finalPrice,'合同价格必须为数字')) {
					return false;
				}
